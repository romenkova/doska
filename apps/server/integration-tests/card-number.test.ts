...
describe("card numbers", () => {
  test("stamps a number on the first push", async () => {
    ...
    expect(pulled(res.changes, "card1")?.number).toBe(1)
  })

  /**
   * A client that edits a card before its number has arrived pushes
   * `number: null` again. The number is the server's: it keeps the one it has,
   * and the clock is left alone since the client always takes the number from
   * the server regardless of stamps.
   */
  test("a later push that omits the number still gets it back", async () => {
    ...
    expect(record?.updatedAt).toBe(editedAt)
